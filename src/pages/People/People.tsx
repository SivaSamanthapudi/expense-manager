...
const People = () => {
  ...
  groups.forEach(g => {
    g.members.forEach(m => {
      ...
      expenses.filter(e => e.groupId === g.id).forEach(e => {
  ...
  useEffect(() => {
    const peers = people
      .filter(p => p.userId)
      .map(p => ({ userId: p.userId!, name: p.name, avatar: p.avatar, email: p.email }));
    if (peers.length > 0) registerPeers(peers);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [people.length]);

  const handleMessage = (p: { name: string; email: string; avatar: string; userId: string | null }) => {
    if (!p.userId) return;
    openChat({ userId: p.userId, name: p.name, avatar: p.avatar, email: p.email });
  };
    ...
    <div className="page-content">
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">{people.length} people across all groups</p>
        </div>
      </div>
      ...
      {people.length === 0 ? (
        ...
        <div className="people-grid">
          {people.map(p => {
            ...
            return (
              <div key={p.userId ?? p.email} className="person-card card card-body">
                <div className="person-header">
                  <div className="person-avatar-wrap">
                    <img src={p.avatar} alt={p.name} className="avatar avatar-lg" />
                    {canChat && (
                      <span className={`person-online-dot ${isOnline ? 'person-online-dot--on' : ''}`} />
                    )}
                  </div>
                  <div className="person-info">
                    ...
                    {canChat && (
                      <p className={`text-xs person-status-label ${isOnline ? 'person-status-label--on' : ''}`}>
                  ...
                  {canChat && (
                    ...
                    >
                      ...
                      {unread > 0 && <span className="btn-message-badge">{unread}</span>}
                    </button>
                  )}
                </div>
                ...
                <div className="person-stats">
                  <div className="person-stat">
                    ...
                    <p className="font-semibold text-sm" style={{ color: '#10b981' }}>₹{p.totalPaid.toLocaleString()}</p>
                  </div>
                  <div className="person-stat">
                    <p className="text-xs text-muted">Total Share</p>
                    <p className="font-semibold text-sm" style={{ color: '#ef4444' }}>₹{p.totalShare.toLocaleString()}</p>
                  </div>
                  <div className="person-stat">
                    <p className="text-xs text-muted">Net</p>
                    <p className="font-semibold text-sm" style={{ color: net >= 0 ? '#10b981' : '#ef4444' }}>
