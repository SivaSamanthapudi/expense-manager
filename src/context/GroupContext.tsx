...
export const GroupProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    // Wait until auth has finished initialising before acting
  ...
  const addGroup = useCallback(
    async (payload: Omit<Group, 'id' | 'createdAt'>) => {
  ...
  const addMember = useCallback(async (member: Omit<Member, 'id'>) => {
    ...
    setGroups((prev) =>
      prev.map((g) => {
        ...
        // For registered users (userId set), dedup by userId; for guests, dedup by non-empty email
        const isDuplicate = member.userId
          ? g.members.some((m) => m.userId === member.userId)
          : member.email
          ? g.members.some(
              (m) =>
                !m.userId &&
    ...
    try {
      ...
      // Always send userId so the backend can link by ID instead of email/mobile
